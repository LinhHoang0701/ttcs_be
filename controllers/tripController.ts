...
// @Route /api/trips
// @Method POST
export const addTrip = asyncHandler(
...
// @Method POST
export const createTripReview = asyncHandler(
  async (req: IUserRequest, res: Response) => {
    const trip = await Trip.findById(req.params.id);
    ...
    if (trip) {
      ...
      trip.ratings =
        trip.reviews?.reduce((acc: any, item: any) => item?.rating + acc, 0) /
        Number(trip.reviews?.length);
